...
async def _check_movie_exists(client: SearchClient, index_name: str, title: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        search_response = await client.multiple_search_async(search_params)
...
async def vote_for_movie(client: SearchClient, movies_index_name: str, votes_index_name: str,
    ...
    try:
        ...
        update_payload = {
            ...
            "indexName": movies_index_name
        }

        update_result = client.partial_update_objects(update_payload)
        ...
        try:
            await client.wait_task_async(update_task_id, index_name=movies_index_name)
...
async def get_movie_by_id(client: SearchClient, index_name: str, movie_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        response = await client.get_object_async(object_id=movie_id, index_name=index_name)
        return response
    except Exception as e:
...
def search_movies_for_vote(client: SearchClient, index_name: str, title: str) -> Dict[str, Any]:
    ...
    try:
        ...
        search_response = client.multiple_search(search_params)
        search_result = search_response.results[0]  # Get first result
...
async def get_all_movies(client: SearchClient, index_name: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # V4 API: Using browse_objects with index_name
        browse_iterator = client.browse_objects_iterator(
            {
                "indexName": index_name,
        ...
        async for hit in browse_iterator:
